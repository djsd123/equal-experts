...
const options = yargs(hideBin(process.argv)).usage('Usage: node index.js [-u, --user [username]] [-f, --freshrun]')
    .options('user', {
        alias: 'u',
    ...
    .option('freshrun', {
        alias: 'f',
        describe: 'List all of the user\'s gists even if listed during a previous run',
        type: 'boolean',
        default: false,
        demandOption: false,
    }).help().parseSync()
...
const getGists = async (user: string, time?: string) => {
    const results = await client.request('GET /users/{username}/gists', {
        username: user,
        since: time,
    }).then(result => {
        console.log(chalk.cyan(figlet.textSync(user, { horizontalLayout: 'default', font: 'Colossal' })))
        result.data.forEach(gist => console.log(
            chalk.magentaBright('Created: ') + new Date(gist.created_at).toDateString() +
            chalk.cyan(','), chalk.green('URL: ') + gist.html_url +
            chalk.cyan(','), chalk.yellow('Description: ') + gist.description
            )
        )
        return
